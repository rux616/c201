...
void MyFloat::Read()
	{
		...
		if (Ch != '.')
			return;
		cin.get(Ch);
		while (isdigit(Ch) && k < MAX_DIGITS + 1)
		{
			Number[k] = Ch - '0';
			cin.get(Ch);
			++ k;
...
const MyFloat MyFloat::operator+(const MyFloat &Other) const
{
	...
	MyFloat Sum;
	if (NumberOfDigits > Other.NumberOfDigits)
		Sum.NumberOfDigits = NumberOfDigits;
	else
		Sum.NumberOfDigits = Other.NumberOfDigits;
	for (int i = MAX_DIGITS;  i > 0; --i)
	{
		if (Number[i] + Other.Number[i] > 9)
		{
			Sum.Number[i] = ((Number[i] - Other.Number[i]) % 10);
			Carry = 1;
		}
		else
		{
			Sum.Number[i] = Number[i] + Other.Number[i];
			Carry = 0;
		}
	}
	return Sum; 
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
					     //  Name of your class definition. Use pathname
